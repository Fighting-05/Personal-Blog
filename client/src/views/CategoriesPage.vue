<template>
  <div>
    <div class="section-header" data-aos="fade-down" style="text-align:center;padding:60px 0 32px">
    ...
    <div class="cat-grid">
      <router-link v-for="(c, i) in categories" :key="c.id" :to="{ path: '/', query: { category: c.slug } }" class="cat-card" :data-aos="'fade-up'" :data-aos-delay="i * 80">
        ...
        <div class="cat-info">
          <h3>{{ c.name }}</h3>
          <p>{{ c.description }}</p>
          <span class="cat-count">{{ c.post_count }} 篇文章</span>
        </div>
...
onMounted(async () => {
  try { categories.value = (await postAPI.getCategories()).data } catch (_) {}
})
</script>
...
.cat-info { flex: 1 }
.cat-info h3 { font-size: 1.05rem; font-weight: 700; color: var(--color-text); margin: 0 0 4px }
.cat-info p { font-size: 0.82rem; color: var(--color-text-muted); margin: 0 0 6px }
.cat-count { font-size: 0.78rem; color: var(--color-accent); font-weight: 600 }
.cat-arrow { color: var(--color-text-muted); font-size: 0.9rem; transition: transform 0.3s }
